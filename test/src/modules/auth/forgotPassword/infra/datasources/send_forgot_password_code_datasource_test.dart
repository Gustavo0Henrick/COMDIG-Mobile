...
main() {
  ...
  late SendForgotPasswordCodeDatasource datasource;

  setUp(() {
    requester = AppRequesterMock();
    datasource = SendForgotPasswordCodeDatasource(requester: requester);
  });

  String email = "email@test.com";

  test("Should call the SendForgotPasswordCode with correct Method", () async {
    when(() async => await requester.post(
          ...
          statusCode: 200,
          requestOptions: RequestOptions(data: {"email": email}),
        ));

    await datasource.sendForgotPasswordCode(email);
  });
  ...
  test("Should return a Success Message when is successful", () async {
    ...
    Response response = await datasource.sendForgotPasswordCode(email);
  ...
  test("Should throw a BadRequestException when the the request is invalid",
    ...
    final result = datasource.sendForgotPasswordCode("email@test.com");
