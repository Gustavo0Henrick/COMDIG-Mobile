...
main() {
  ...
  late GetEventsListDatasource datasource;

  setUp(() {
    requester = AppRequesterMock();
    datasource = GetEventsListDatasource(requester: requester);
  });

  String query = "Presidente Prudente";

  test("Should call the GetEventsList with correct Method", () async {
    ...
    await datasource.getEventsList(query);
  });
  ...
  test("Should return a Success Message when is successful", () async {
    ...
    Response response = await datasource.getEventsList(query);
  ...
  test("Should throw a BadRequestException when the the request is invalid",
    ...
    final result = datasource.getEventsList(query);
