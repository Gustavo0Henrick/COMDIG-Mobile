...
main() {
  ...
  late GetReviewsDatasource datasource;

  setUp(() {
    requester = AppRequesterMock();
    datasource = GetReviewsDatasource(requester: requester);
  });
  String companyId = "companyId";

  DateTime date = DateTime.now();

  test("Should call the GetReviews with correct Method", () async {
    ...
    await datasource.getReviews(companyId);
  });
  ...
  test("Should return a Success Message when is successful", () async {
    ...
    Response response = await datasource.getReviews(companyId);
  ...
  test("Should throw a BadRequestException when the the request is invalid",
    ...
    final result = datasource.getReviews(companyId);
