...
main() {
  ...
  late AddFavoriteDatasource datasource;

  setUp(() {
    requester = AppRequesterMock();
    datasource = AddFavoriteDatasource(requester: requester);
  });

  String companyId = "companyId";

  test("Should call the AddFavoriteById with correct Method", () async {
    ...
    await datasource.addFavoriteById(companyId);
  });
  ...
  test("Should return a Success Message when is successful", () async {
    ...
    Response response = await datasource.addFavoriteById(companyId);
  ...
  test("Should throw a BadRequestException when the the request is invalid",
    ...
    final result = datasource.addFavoriteById(companyId);
