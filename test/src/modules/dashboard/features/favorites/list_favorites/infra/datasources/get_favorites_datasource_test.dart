...
main() {
  ...
  late GetFavoritesDatasource datasource;

  setUp(() {
    requester = AppRequesterMock();
    datasource = GetFavoritesDatasource(requester: requester);
  });

  String userId = "userId";

  test("Should call the getFavorites with correct Method", () async {
    ...
    await datasource.getFavorites(userId);
  });
  ...
  test("Should return a Success Message when is successful", () async {
    ...
    Map<String, dynamic> listFavoriteModelsExpected = {
      "listFavoriteModels": [
        {
          "id": "id",
          "name": "name",
    ...
    Response response = await datasource.getFavorites(userId);
  ...
  test("Should throw a BadRequestException when the the request is invalid",
    ...
    final result = datasource.getFavorites(userId);
